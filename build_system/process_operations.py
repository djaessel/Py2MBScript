...
def get_identifier_value(strx, tag_uses):
  underscore_pos = strx.find("_")
  result = -1
  if (underscore_pos > 0):
    tag_str = strx[0:underscore_pos]
    id_str  = strx[underscore_pos + 1:len(strx)]
    (tag_type, id_no) = get_id_value(tag_str,id_str,tag_uses)
    if (tag_type > 0):
      if (id_no < 0):
        print("Error: Unable to find object:" + strx)
      else:
        result = id_no | (tag_type << op_num_value_bits)
    else:
      print("Error: Unrecognized tag:" +tag_str + "in object:" + strx)
  else:
    print("Error: Invalid object:" + strx + ".Variables should start with $ sign and references should start with a tag")
  return result
